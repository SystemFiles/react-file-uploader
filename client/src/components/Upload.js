...
function Upload (props) {
  ...
  // Get props
  const { onUpload, isUploading, done, error, progress } = props
      ...
      <Grid item xs={12} sm={5} className='Upload-List'>
        <div className='Upload-List-Container'>
          {state.selectedFiles.length === 0 ? (
            ...
            state.selectedFiles.map((file) => <UploadListItem key={uuid()} file={file} onClick={deleteSelectedFile} />)
          )}
        </div>
      </Grid>
      <Grid item xs={12} className='Upload-Submit-Container'>
        <button type='submit' disabled={isUploading || done || error} onClick={handleUploadFiles}>
          {isUploading ? (
